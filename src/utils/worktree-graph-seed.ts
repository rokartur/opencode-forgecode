 ...
 */
export async function seedWorktreeGraphScope(
  options: SeedWorktreeGraphScopeOptions,
): Promise<SeedResult> {
  ...
  if (
    sourceMetadata.indexedFileCount === undefined ||
    sourceMetadata.indexedMaxMtimeMs === undefined
  ) {
  ...
  const sourceGraphHealthy = validateGraphDatabaseHealth(
    sourceGraphDbPath,
    sourceMetadata.indexedFileCount ?? 0,
  );
