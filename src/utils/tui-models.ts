...
export async function fetchAvailableModels(api: TuiPluginApi): Promise<FetchModelsResult> {
  try {
    ...
    if (result.error) {
      const errorMsg =
        (result.error as { message?: string })?.message || "Failed to fetch providers";
...
export function recordRecentModel(
  ...
  try {
    ...
    const updated = [modelFullName, ...existing.filter((m) => m !== modelFullName)].slice(
      0,
      RECENT_MODELS_MAX,
    );
