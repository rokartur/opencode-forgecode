        ...
        try {
          for (const raw of this.decoder.push(chunk)) {
            ...
            if (!handshakeDone) {
              if (msg?.kind === "hello") {
                if (!msg.accepted) {
                  fail(new Error(`SocketTransport: leader rejected handshake: ${msg.reason ?? "unknown"}`));
                  return;
                }
                if (msg.version !== GRAPH_IPC_VERSION) {
                  fail(new Error(`SocketTransport: version mismatch (leader=${msg.version}, self=${GRAPH_IPC_VERSION})`));
                  return;
                }
                handshakeDone = true;
                clearTimeout(timer);
