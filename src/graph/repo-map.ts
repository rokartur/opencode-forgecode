...
export class RepoMap {
  ...
  private prepareStatements(): void {
    this.stmts = {
      ...
      deleteTokenSignaturesByFileId: this.db.prepare(
        "DELETE FROM token_signatures WHERE file_id = ?",
      ),
      deleteTokenFragmentsByFileId: this.db.prepare(
        "DELETE FROM token_fragments WHERE file_id = ?",
      ),
      deleteExternalImportsByFileId: this.db.prepare(
        "DELETE FROM external_imports WHERE file_id = ?",
      ),
      ...
      getEdgesTargetIds: this.db.prepare(
        "SELECT target_file_id FROM edges WHERE source_file_id = ?",
      ),
      // FTS search
  ...
  private initSchema(): void {
    ...
    try {
      const ftsCount = this.db.prepare("SELECT COUNT(*) as c FROM symbols_fts").get() as
        | { c: number }
        | undefined;
  ...
  async indexFile(filePath: string): Promise<void> {
    ...
    for (const imp of outline.imports) {
      ...
      if (isRelative) {
        ...
        if (resolvedSource) {
          const resolvedFile = this.stmts.getFileByPath.get(resolvedSource) as
            | IndexedFile
            | undefined;
          if (resolvedFile) {
      ...
      for (const ref of unresolved) {
        ...
        if (matches.length >= 1) {
          if (ref.import_source) {
            ...
            if (pathMatch) {
              this.db.run("UPDATE refs SET source_file_id = ? WHERE id = ?", [
                pathMatch.file_id,
                ref.id,
              ]);
              continue;
  ...
  getTopFiles(limit = 20): TopFileResult[] {
    const files = this.db
      .prepare("SELECT * FROM files ORDER BY pagerank DESC LIMIT ?")
      .all(limit) as DbFile[];
    return files.map((f) => ({
      path: f.path,
      pagerank: f.pagerank,
