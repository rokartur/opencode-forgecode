...
- Ready: ${graphService.ready}`
				...
				if (!graphService || !graphService.ready) {
					const status = ctx.kvService.get<{
						state: string
						message?: string
						stats?: { files: number; symbols: number; edges: number; calls: number }
					}>(ctx.projectId, 'graph:status')
					if (status) {
						...
						const stateMsg =
							status.state === 'indexing' && status.message ? status.message : `State: ${status.state}`
						const statsMsg = status.stats
							? `\nStats so far:\n- Files: ${status.stats.files}\n- Symbols: ${status.stats.symbols}`
							: ''
...
- Ready: ${graphService.ready}`
		...
		'graph-query': tool({
			description:
				'Query file-level graph information (dependencies, dependents, co-changes, change impact, etc.)',
			args: {
				action: z
					.enum([
						'top_files',
					...
					switch (args.action) {
						case 'top_files': {
							...
							return results
								.map(
									r =>
										`**${r.path}** (PageRank: ${r.pagerank.toFixed(4)}, ${r.lines} lines, ${r.symbols} symbols)`,
								)
								.join('\n')
						}
						...
						case 'file_symbols': {
							...
							return results
								.map(
									r => `- ${r.name} (${r.kind})${r.isExported ? ' [exported]' : ''} [line ${r.line}]`,
								)
								.join('\n')
						}
					...
					switch (args.action) {
						...
						case 'search': {
							...
							return filtered
								.map(
									r =>
										`**${r.name}** in ${r.path}:${r.line} (${r.kind})${r.isExported ? ' [exported]' : ''}`,
								)
								.join('\n')
						}
		...
		'graph-analyze': tool({
			description:
				'Analyze code quality (unused exports, duplication, near-duplicates, orphan files, circular dependencies)',
			args: {
				action: z
					.enum(['unused_exports', 'duplication', 'near_duplicates', 'orphan_files', 'circular_deps'])
				...
				try {
					switch (args.action) {
						case 'unused_exports': {
							...
							return results.map(r => `- **${r.name}** in ${r.path}:${r.line} (${r.kind})`).join('\n')
						}
						...
						case 'orphan_files': {
							...
							return results
								.map(
									r =>
										`- **${r.path}** (${r.language}, ${r.lineCount} lines, ${r.symbolCount} symbols)`,
								)
								.join('\n')
						}
