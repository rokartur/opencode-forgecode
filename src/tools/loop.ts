...
export async function setupLoop(ctx: ToolContext, options: LoopSetupOptions): Promise<string> {
  ...
  if (!options.worktree) {
    ...
    logger.log(
      `loop: worktree created at ${worktreeInfo.directory} (branch: ${worktreeInfo.branch})`,
    );
    ...
    logger.log(
      `loop: graph seed ${seedResult.seeded ? "reused" : "skipped"} (${seedResult.reason})`,
    );
  ...
  if (options.worktree) {
    try {
      ...
      if (waitResult === "timeout") {
        ...
        logger.log(
          `setupLoop: graph ready (${waitResult.state}) for worktree ${loopContext.directory}`,
        );
      }
    } catch (err) {
  ...
  return {
    loop: tool({
      ...
      execute: async (args, context) => {
        ...
        const sessionTitle =
          args.title.length > 60 ? `${args.title.substring(0, 57)}...` : args.title;
        const loopModel =
    ...
    "loop-cancel": tool({
      ...
      execute: async (args) => {
        ...
        logger.log(
          `loop-cancel: cancelled loop for session=${state.sessionId} at iteration ${state.iteration}`,
        );
    ...
    "loop-status": tool({
      ...
      execute: async (args) => {
        ...
        if (args.restart) {
          ...
          const { match: stoppedState, candidates } = findPartialMatch(
            args.name,
            allStates,
            (s) => [s.loopName, s.worktreeBranch],
          );
        ...
        if (!args.name) {
          ...
          if (recent.length > 0) {
            ...
            if (recent.length > 10) {
              lines.push(
                `   ... and ${recent.length - 10} more. Use loop-status <name> for details.`,
              );
              lines.push("");
            }
          }
          ...
          lines.push(
            "Use loop-status <name> for detailed info, or loop-cancel <name> to stop a loop.",
          );
          return lines.join("\n");
        }
        ...
        if (!state.active) {
          ...
          const durationStr = formatDuration(
            computeElapsedSeconds(state.startedAt, state.completedAt),
          );
