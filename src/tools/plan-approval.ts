  ...
  return async (
    ...
    if (input.tool === "question") {
      ...
      if (options) {
        ...
        const hasExecuteHere = labels.some(
          (l) => l === "execute here" || l.startsWith("execute here"),
        );
        const isPlanApproval =
          hasExecuteHere || PLAN_EXECUTION_LABELS.every((l) => labels.includes(l));
        if (isPlanApproval) {
    ...
    logger.log(
      `Loop: blocked ${input.tool} tool in ${state.phase} phase for session ${input.sessionID}`,
    );
