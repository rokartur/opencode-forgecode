	...
	try {
		...
		if (argv.name) {
			...
			const restartableLoops = loops.filter(
				l => l.state.active || (l.state.terminationReason && l.state.terminationReason !== 'completed'),
			)
		...
		).run(row.project_id, sessionKey, JSON.stringify(state.loopName), now + 30 * 24 * 60 * 60 * 1000, now)
