...
export async function run(argv: StatusArgs): Promise<void> {
	...
	try {
		if (argv.listWorktrees) {
			const rows = db
				.prepare('SELECT key, data FROM project_kv WHERE key LIKE ? AND expires_at > ?')
				.all('loop:%', Date.now()) as Array<{ key: string; data: string }>
		...
		if (loopName) {
			...
			const resolvedLoopName =
				matchedLoop.type === 'active' ? matchedLoop.loop.loopName : matchedLoop.loop.state.loopName
			...
			if (matchedLoop.type === 'active') {
				...
				const sessionStatus = await tryFetchSessionStatus(
					argv.server ?? 'http://localhost:5551',
					state.sessionId,
					state.worktreeDir!,
				)
				...
				const sessionOutput = await tryFetchSessionOutput(
					argv.server ?? 'http://localhost:5551',
					state.sessionId,
					state.worktreeDir!,
				)
			...
			if (activeLoops.length > 0) {
				...
				for (const loop of activeLoops) {
					...
					console.log(
						`    Phase: ${loop.phase}  Iteration: ${iterStr}  Duration: ${durationStr}  Audit: ${audit}`,
					)
					console.log('')
				}
			...
			if (recentLoops.length > 0) {
				...
				if (recentLoops.length > limit) {
					console.log(
						`  ... and ${recentLoops.length - limit} more. Use 'oc-forgecode loop status <name>' for details.`,
					)
					console.log('')
				}
			}
