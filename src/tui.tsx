...
function cancelLoop(projectId: string, loopName: string): string | null {
  ...
  try {
    ...
    const row = db
      .prepare(
        "SELECT data, project_id FROM project_kv WHERE project_id = ? AND key = ? AND expires_at > ?",
      )
    ...
    db.prepare(
      "UPDATE project_kv SET data = ?, updated_at = ? WHERE project_id = ? AND key = ?",
    ).run(JSON.stringify(updatedState), now, projectId, key);
...
async function restartLoop(
  ...
  try {
    ...
    if (state.active) {
      ...
      db.prepare("DELETE FROM project_kv WHERE project_id = ? AND key = ?").run(
        projectId,
        oldSessionKey,
      );
    }
    ...
    db.prepare(
      "UPDATE project_kv SET data = ?, updated_at = ? WHERE project_id = ? AND key = ?",
    ).run(JSON.stringify(newState), now, projectId, key);
...
function PlanViewerDialog(props: {
  ...
  const [executionModel, setExecutionModel] = createSignal<string>(
    props.initialExecutionModel ?? "",
  );
  ...
  const hasInitialOverrides =
    props.initialExecutionModel !== undefined || props.initialAuditorModel !== undefined;
    ...
    switch (matchedLabel) {
      case "New session": {
        ...
        try {
          ...
          const { parseModelString, retryWithModelFallback } =
...
function LoopDetailsDialog(props: {
  ...
  const handleCancel = () => {
    ...
    props.api.ui.toast({
      message: sessionId
        ? `Cancelled loop: ${currentLoop().name}`
        : `Loop ${currentLoop().name} is not active`,
...
function Sidebar(props: { api: TuiPluginApi; opts: TuiOptions; sessionId?: string }) {
  ...
  const [graphStatusFormatted, setGraphStatusFormatted] = createSignal<ReturnType<
    typeof formatGraphStatus
  > | null>(null);
  const [graphStatusRaw, setGraphStatusRaw] = createSignal<ReturnType<
    typeof readGraphStatus
  > | null>(null);
  const theme = () => props.api.theme.current;
  const directory = props.api.state.path.directory;
  const pid = resolveProjectId(directory);
  ...
  function refreshSidebarData() {
    ...
    const visible = states.filter(
      (l) => l.active || (l.completedAt && new Date(l.completedAt).getTime() > cutoff),
    );
    visible.sort((a, b) => {
