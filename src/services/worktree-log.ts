	...
	try {
		...
		if (context.sandbox) {
			if (context.sandboxHostDir) {
				...
				permissionPath = mappedPath.startsWith('/workspace/') || mappedPath === '/workspace' ? mappedPath : null
			} else {
 ...
 */
export function writeWorktreeCompletionLog(payload: WorktreeCompletionLogPayload, logger?: Logger): boolean {
