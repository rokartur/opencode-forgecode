...
export function createLoopEventHandler(
  ...
  async function commitAndCleanupWorktree(
    state: LoopState,
  ): Promise<{ committed: boolean; cleaned: boolean }> {
    if (!state.worktree) {
    ...
    if (state.worktreeDir && state.worktreeBranch) {
      try {
        ...
        cleaned = true;
        logger.log(
          `Loop: removed worktree ${state.worktreeDir}, branch ${state.worktreeBranch} preserved`,
        );
        ...
        if (state.worktreeDir && projectId && dataDir) {
          try {
            ...
            logger.error(
              `Loop: failed to delete graph cache for worktree ${state.worktreeDir}`,
              err,
            );
          }
        }
      ...
      try {
        ...
        try {
          ...
          if (hasActiveWork) {
            lastActivityTime.set(loopName, Date.now());
            logger.log(
              `Loop watchdog: loop ${loopName} has active work (${status}), resetting timer`,
            );
            return;
          }
        } catch (err) {
  ...
  function getStallInfo(
    loopName: string,
  ): { consecutiveStalls: number; lastActivityTime: number } | null {
    const lastActivity = lastActivityTime.get(loopName);
  ...
  async function terminateLoop(loopName: string, state: LoopState, reason: string): Promise<void> {
    ...
    logger.log(
      `Loop terminated: reason="${reason}", loop="${state.loopName}", iteration=${state.iteration}`,
    );
    ...
    if (reason === "completed" && state.worktree) {
      ...
      if (completionResult) {
        ...
        if (written) {
          ...
          logger.error(
            `Loop: failed to write worktree completion log to ${completionResult.hostPath}`,
          );
        }
      } else {
  ...
  async function getLastAssistantInfo(
    ...
    try {
      ...
      if (!lastAssistant) {
        ...
        logger.log(
          `Loop: no assistant message found in session ${sessionId}, last message role: ${role}`,
        );
      ...
      const error =
        lastAssistant.info.error?.data?.message ?? lastAssistant.info.error?.name ?? null;
  ...
  async function rotateSession(loopName: string, state: LoopState): Promise<string> {
    ...
    v2Client.session
      .delete({ sessionID: oldSessionId, directory: state.worktreeDir })
      .catch((err) => {
    ...
    if (isModelError) {
      ...
      if (nextErrorCount >= MAX_RETRIES) {
        await terminateLoop(
          loopName,
          currentState,
          `error_max_retries: assistant error: ${assistantError}`,
        );
        return null;
      }
      loopService.setState(loopName, {
  ...
  async function checkCompletionAndTerminate(
    ...
    if (!currentState.audit || auditCount >= minAudits) {
      if (loopService.hasOutstandingFindings(currentState.worktreeBranch)) {
        logger.log(
          `Loop: completion promise detected but outstanding review findings remain, continuing`,
        );
        return false;
      }
    ...
    logger.log(
      `Loop: completion promise detected but only ${auditCount}/${minAudits} audits performed, continuing`,
    );
    return false;
  }
  ...
  async function handleCodingPhase(loopName: string, _state: LoopState): Promise<void> {
    ...
    if (currentState.completionSignal) {
      ...
      const errorResult = await detectAndHandleAssistantError(
        loopName,
        currentState,
        assistantError,
        "coding",
      ...
      if (
        await checkCompletionAndTerminate(
          loopName,
          currentState,
          textContent,
          currentState.auditCount ?? 0,
        )
      )
        return;
    }

    currentState = resetErrorCountIfNeeded(
      loopName,
      currentState,
  ...
  async function handleAuditingPhase(loopName: string, _state: LoopState): Promise<void> {
    ...
    if (
      (currentState.maxIterations ?? 0) > 0 &&
      nextIteration > (currentState.maxIterations ?? 0)
    ) {
      await terminateLoop(loopName, currentState, "max_iterations");
      return;
    }
  ...
  async function onEvent(input: {
    ...
    await withStateLock(loopName, async () => {
      ...
      if (state.sessionId !== sessionId) {
        logger.log(
          `Loop: ignoring stale idle event for session ${sessionId} (current: ${state.sessionId})`,
        );
        return;
      }
