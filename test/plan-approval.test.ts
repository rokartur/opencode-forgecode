...
describe("Plan Approval Tool Interception", () => {
    ...
    if (tool === "question") {
      ...
      if (options) {
        ...
        if (isPlanApproval) {
          ...
          const matchedLabel = PLAN_APPROVAL_LABELS.find(
            (l) => answer === l || answer.startsWith(l),
          );
  ...
  test('Detects plan approval question and handles "Execute here" with abort', () => {
    ...
    expect(output.output).toContain("Switching to forge agent");
    expect(output.output).not.toContain("<system-reminder>");
  });
...
describe("Execute here bypass", () => {
  ...
  test("Execute here bypasses directive injection and triggers abort", async () => {
    ...
    await hook({ tool: "question", sessionID, callID: "test-call", args }, output);
  ...
  test("session.idle event triggers promptAsync for pending execution", async () => {
    ...
    await afterHook({ tool: "question", sessionID, callID: "test-call", args }, output);
    ...
    expect(callArgs.parts[0].text).toContain("The muse agent has created an implementation plan");
  ...
  test("New session path reads plan from session-scoped KV and deletes after dispatch", async () => {
    ...
    await hook({ tool: "question", sessionID: testSessionID, callID: "test-call", args }, output);
