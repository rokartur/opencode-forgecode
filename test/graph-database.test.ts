...
describe('Graph database initialization', () => {
    ...
    `)
		...
		const db2 = initializeGraphDatabase(testProjectId, testDataDir)
		const tableInfo = db2.prepare('PRAGMA table_info(semantic_summaries)').all() as Array<{
			name: string
			pk: number
		}>
	...
	test('should preserve data when table already has correct schema', () => {
		...
		db.run(
			`INSERT INTO files (path, mtime_ms, language, line_count, symbol_count, pagerank, is_barrel, indexed_at) VALUES ('/test.ts', 123, 'typescript', 10, 1, 0.5, 0, 123)`,
		)
		const fileId = db.prepare('SELECT last_insert_rowid() AS id').get() as { id: number }

		// Then create a symbol
		db.run(
			`INSERT INTO symbols (file_id, name, kind, line, end_line, is_exported) VALUES (?, 'test', 'function', 1, 5, 1)`,
			[fileId.id],
		)
...
describe('Graph database corruption recovery', () => {
	...
	test('should be able to insert and read data after graph DB recovery', () => {
		...
		// Read it back
		const result = recoveredDb.prepare('SELECT name FROM symbols WHERE file_id = ?').get(fileId.id) as {
			name: string
		}
