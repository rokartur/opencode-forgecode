...
describe('bundled sample config', () => {
	...
	test('bundled forge-config.jsonc is valid JSONC and parses successfully', () => {
		...
		const _config = loadPluginConfig()
	...
	test('bundled config includes all supported top-level keys', () => {
		...
		expect(parsed.intentGate).toBeDefined()
		expect(parsed.contextInjection).toBeDefined()
		expect(parsed.restrictedShell).toBeDefined()
		expect(parsed.telemetry).toBeDefined()
		expect(parsed.harness).toBeDefined()
