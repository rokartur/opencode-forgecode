...
test('scoped external package names preserved', async () => {
	...
	writeFileSync(
		fileA,
		`import type { Logger } from '@opencode-ai/sdk'\nimport { resolve } from '@types/node/path'\n\nexport const app = null`,
	)
...
test('token extraction works when repo root differs from process cwd', async () => {
	...
	const tokenSigs = db
		.prepare('SELECT * FROM token_signatures WHERE file_id = (SELECT id FROM files WHERE path = ?)')
		.all('a.ts') as Array<{
		id: number
		file_id: number
		name: string
...
test('getFileSymbols returns one entry per declaration (no duplicates)', async () => {
	...
	writeFileSync(
		fileA,
		`export function createService(ttlSeconds?: number) {
  return null
}`,
	)
